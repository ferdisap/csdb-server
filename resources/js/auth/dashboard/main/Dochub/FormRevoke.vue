...
const submitting = (formData: any) => {
  ...
  return new Promise<any>((resolve, reject) => {
    axios.post(`/dochub/token/revoke`, formData)
      .then(response => {
  ...
  <Dialog :open="open" @update:open="open = $event">
    ...
    <DialogContent class="sm:max-w-[425px]">
      ...
      <form id="dialogForm" @submit.prevent="onSubmited">
        <FormField v-slot="{ value, handleChange }" name="revoke">
          <FormItem class="mb-4">
            <FormControl class="flex">
              <div class="flex items-center">
                <Checkbox class="mr-2" id="revoke_token_form" :model-value="value"
                  @update:model-value="handleChange" />
                <FormLabel for="revoke_token_form">Revoke</FormLabel>
              </div>
            </FormControl>
            <FormDescription>
              The revoked token will not be able to access dochub.
            </FormDescription>
            <FormMessage />
          </FormItem>
        </FormField>
