...
const emit = defineEmits(['clientUpdated']) // ✅ wajib dideklarasikan
...
const submitting = (formData: any) => {
  ...
  return new Promise<any>((resolve, reject) => {
    ...
    //   }
    // });
    axios.post(`/oauth-client/${id}/update`, formData)
      .then(response => {
...
const form = useForm({
  initialValues: {
    revoked: props.data.client.revoked ? true : false
  }
});
...
watch(
  ...
  (newData) => {
    form.setValues({
      revoked: newData.client.revoked ? true : false
    })
  },
...
<template>
  <Dialog :open="open" @update:open="open = $event">
    ...
    <DialogContent class="sm:max-w-[425px]">
      <DialogHeader>
        <DialogTitle>Revoke client id</DialogTitle>
        <DialogDescription>
          ...
          <FormItem class="mb-4">
            <FormControl class="flex">
              <div class="flex items-center">
                <Checkbox class="mr-2" id="revoked_client_form" :model-value="value"
                  @update:model-value="handleChange" />
                <FormLabel for="revoked_client_form">Revoke</FormLabel>
              </div>
            </FormControl>
            <FormDescription>
              The revoked client will not be able to use OAuth authentication.
            </FormDescription>
            <FormMessage />
          </FormItem>
        </FormField>
