...
const form = useForm({
  initialValues: {
    app_name: props.data.client.name
  }
});
      ...
      <form id="dialogForm" @submit.prevent="onSubmited">
        <FormField v-slot="{ componentField }" name="app_name">
          <FormItem class="mb-4">
            ...
            <FormControl>
              <Input type="text" placeholder="Example App" v-bind="componentField" :default-value="props.data.client.name"/>
            </FormControl>
            <FormDescription>
              This name is a third party application's name which can access some feature of this application.
            </FormDescription>
            <FormMessage />
          </FormItem>
        </FormField>
