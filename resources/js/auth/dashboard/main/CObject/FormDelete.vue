...
const emit = defineEmits(["deletedCObject"]);
...
const submitting = (formData: any) => {
  ...
  return new Promise<any>((resolve, reject) => {
    axios.post(`/csdb-object/${id}/delete`, formData)
...
const onSubmited = form.handleSubmit((values: any) => {
  ...
  submitting(values)
    .then((data: any) => {
      open.value = false;
      const cobject = castToCObject(data.csdb.objects[0]);
      emit('deletedCObject', cobject);
    ...
    <DialogTrigger as-child>
      <TooltipProvider>
        <Tooltip>
          ...
          <TooltipContent class="border border-light dark:border-dark bg-light dark:bg-dark">Delete Csdb Object</TooltipContent>
        </Tooltip>
      </TooltipProvider>
    </DialogTrigger>
    <DialogContent class="sm:max-w-[425px]">
      <DialogHeader>
        <DialogTitle>Delete CSDB Object</DialogTitle>
        <DialogDescription>
      ...
      <form id="dialogForm" @submit.prevent="onSubmited">
        <FormField v-slot="{ value, handleChange }" name="deleted">
          <FormItem class="mb-4">
            ...
            <FormDescription>
              The csdb object will be put on the trash for 30 Days automatically permanently deleted.
            </FormDescription>
            <FormMessage />
          </FormItem>
        </FormField>
        <FormField v-slot="{ value, handleChange }" name="permanent">
          <FormItem class="mb-4">
            <FormControl class="flex">
              <div class="flex items-center">
                <Checkbox class="mr-2" id="permanent_deleted_token_form" :model-value="value"
                  @update:model-value="handleChange" />
                <FormLabel for="permanent_deleted_token_form">Permanent</FormLabel>
              </div>
            </FormControl>
            <FormDescription>
              Permanently deletion will not be able to restore.
            </FormDescription>
            <FormMessage />
          </FormItem>
        </FormField>
